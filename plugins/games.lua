--[[
الالعاب ل سورس كلاو
تم تعديل الملف واضافه الالعاب من قبل باسل اباظه (@XB_0B)
--]]
...
t9 =[[
•ماعدد الدول العربيه التي توجد في قارة افريقيا؟ 
...
t17 =[[
•اصفر اللون سارق عقول اهل الكون وحارمهم لذةالنوم؟
...
t28 =[[
•ما اسم المعدن الموجود في الحاله السائله؟
...
•اختر اليد الصحيحه لإستخراج المحيبس الصحيح 
...
return '️•اول من يكتب هذا الشكل يفوز ››    `'..(redis:get(max..':Set_Smile:'..msg.chat_id_))..'`'
end
end
...
katu = {'قرد','حصان','ارنب','ثعبان','نمله','قطه','كلب','ثور','ماعز','خروف','سلحفات','حوت','ورده','نخله','شجره','شمس','هلال','كامره','حلزون','مسدس','طياره','سياره','كرز','بطيخ','فروله','منزل','كره','نجمه','ساعه','راديو','باب','قارب','دجاجه','بطريق','ضفدع','بومه','نحله','ديك','جمل','بقره','دولفين','تمساح','قرش','نمر','اخطبوط','سمكه','خفاش','اسد','فأر','ذئب','فراشه','عقرب','زرافه','قنفذ','تفاحه','باذنجان'}
name = katu[math.random(#katu)]
redis:set(max..':Set_Name_Meant:'..msg.chat_id_,name)
name = string.gsub(name,'قرد','🐒')
name = string.gsub(name,'حصان','🐎')
name = string.gsub(name,'ارنب','🐇')
name = string.gsub(name,'ثعبان','🐍')
...
return '•اول من يكتب معنى الايموجي يفوز { '..name..' }'
end
...
return '*•عدد النقاط التي ربحتها هي » { '..points..' }\n•تسطيع بيع نقاطك والحصول على (100) رساله مقابل كل نقطه من النقاط *\n'
else
...
if MsgText[2] == "0" then
return '•هناك خطأ عزيزي  \n•يجب ان يكون البيع على الاقل 1 من النقاط . 'end
local points = redis:get(max..':User_Points:'..msg.chat_id_..msg.sender_user_id_)
if tonumber(MsgText[2]) > tonumber(points) then
return '•عذرا ليس لديك نقاط بهذا العدد لبيعهم' 
end
if points == "0" then
...
•لتفعيل الالعاب او تعطيلها .
ـ  ▿ تفعـيل ⟷ تعطيل ⇦ الالعاب
  ▿ ▿ ▿ ▿  ▿ ▿ ▿ ▿ ▿ ▿ ▿ ▿ ▿ ▿ ▿ ▿
...
for k, v in pairs(list) do 
if msg.text ~= v then
sendMsg(msg.chat_id_,msg.id_,'اجابتك خاطئه')     
redis:del(max.."GAME:S"..msg.chat_id_)
redis:del(max.."GAME:CHER"..msg.chat_id_)
return false  
end
...
redis:incrby(max..':User_Points:'..msg.chat_id_..data.id_,5)
sendMsg(msg.chat_id_,msg.id_,'*•مبروك فزت ومعنا وخمنت الرقم الصحيح\n•تم اضافة { 5 } من النقاط *\n')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
