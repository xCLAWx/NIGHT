...
if redis:get(max.."getidstatus"..msg.chat_id_) == "Photo" then
    if data.photos_[0] then 
        ...
        sendPhoto(msg.chat_id_,msg.id_,data.photos_[0].sizes_[1].photo_.persistent_id_,'⌁ NAME ⇨'..Namei..'  \n⌁ USE ⇨ '..UserNameID..' .\n⌁ MSG⇨ '..msgs..' .\n⌁ MSG⇨ '..msg.TheRank..' .\n⌁ iD ⇨ '..msg.sender_user_id_..' .\n⌁ Ch ⇨ @abazaa .',dl_cb,nil)
        else
...
if MsgText[2] == "ردود نايت"   then return unlock_replay(msg) end
if MsgText[2] == "الاذاعه"  then return unlock_brod(msg) end
if MsgText[2] == "ايدي نايت"   then return unlock_IID(msg) end
if MsgText[2] == "ترحيب نايت"  then return unlock_Welcome(msg) end
...
if MsgText[2] == "ردود نايت"   then return lock_replay(msg) end
if MsgText[2] == "الاذاعه"  then return lock_brod(msg) end
if MsgText[2] == "ايدي نايت"   then return lock_IID(msg) end
if MsgText[2] == "ترحيب نايت"  then return lock_Welcome(msg) end
...
return '-اصدار سورس نايت  : *v'..version..'* \n♾'
end
...
sendMsg(msg.chat_id_,msg.id_,'↫ عذرا اسم الملف غير مدعوم للنظام او لا يتوافق مع سورس نايت يرجى جلب الملف الاصلي الذي قمت بسحبه وبدون تعديل ع الاسم\n♽')
end  
else
...
send_key(msg.sender_user_id_,'  [-قناة سورس : نايت ل](https://t.me/abazaa)',nil,inline,msg.id_)
return false
end
...
[●  الاوامر العامـه‌‏ :]
  ...
  ●   اوامر الرد ◄ لاضـافة‌‏ رد مـعين .
    ...
    ● السورس ◄ سورس البوت
    ● المطور  ◄ مطور البوت
 ●اوامر المشرفين ⇦ لعرض اوامر الاشراف
   ● ● ●●  ● ● ● ● ● ● ● ● ● ●● ●
...
return [[
مطـور السورس  ⇦ : [باسـل](t.me/xb_0b)

مطـور السورس  ⇦ : [سـونـيـك](t.me/Soonicxx)

قناة السورس   ⇦ :[ســورس نــايت](t.me/abazaa)
 🎴
]]
end
...
local colors = {'blue','green','yellow','magenta','Oabazaae','DarkOabazaae','red'}
...
local su = {
  ...
  local sh = {
  "انت شخص عادي مطوري اهم.",
  }
    local bb = {
  "انت المطور اؤمر انفذ.",
  }
  local sss = {
  "تمام .",
  "بخيرر يقلبي .",
...
return sendMsg(msg.chat_id_,msg.id_,sh[math.random(#bb)])
elseif not msg.SudoUser and Text== Bot_Name and not Text2 then 
return sendMsg(msg.chat_id_,msg.id_,sh[math.random(#sh)]) 
...
elseif Text== "مح" then return sendMsg(msg.chat_id_,msg.id_,"يععع.")
elseif Text== "باسل" then return sendMsg(msg.chat_id_,msg.id_,"[مطور السورس .](t.me/xb_1b")
elseif Text== "سونيك" then return sendMsg(msg.chat_id_,msg.id_,"[مطور السورس .](t.me/Soonicxx")
elseif Text== "بعشقك"  then return sendMsg(msg.chat_id_,msg.id_,"بموت ف امك .")
elseif Text== "وينك"  then return sendMsg(msg.chat_id_,msg.id_,"بقلبك .")
elseif Text== "مطور السورس"  then return sendMsg(msg.chat_id_,msg.id_,"[بــاســل .](t.me/xb_1b.")
elseif Text== "رفع خادم"  then return sendMsg(msg.chat_id_,msg.id_,"- اهــلا عزيزي\n- تم رفع العضو بنجاح لصيبح خادم\n-لقد اصبح من فئة الخدم\n↯️")
elseif Text== "تنزيل خادم"  then return sendMsg(msg.chat_id_,msg.id_,"- اهــلا عزيزي\n- تم تنزيله من فئة الخدم\n- ولقد تم ارجاعه الى عضو \n↯️")
elseif Text== "رفع كلب"  then return sendMsg(msg.chat_id_,msg.id_,"- اهــلا عزيزي\n- تم رفع العضو كلب بنجاح\n- لقد تمت إضافته إلى قائمه الكلاب \n↯️")
elseif Text== "رفع وتكه"  then return sendMsg(msg.chat_id_,msg.id_,"- اهــلا عزيزي\n- تم رفع العضو وتكه بنجاح\n-  تم إضافته الي قائمة الوتك\n↯️")
